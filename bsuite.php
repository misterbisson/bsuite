...
function widget_any_posts_categories( $number , $selected = array() ){
	...
	foreach( $items as $item ){
		$list[] = '<label for="bsuite-any-posts-categories-'. $item->term_id .'-'. $number .'"><input class="checkbox" type="checkbox" value="'. $item->term_id .'" '.( in_array( $item->term_id, (array) $selected ) ? 'checked="checked"' : '' ) .' id="bsuite-any-posts-categories-'. $item->term_id .'-'. $number .'" name="bsuite-any-posts['. $number .'][categories][]" /> '. $item->name .'</label>';
	}
